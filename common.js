  ...
  export async function loadMembershipApplications(containerId) {
    ...
    try {
      ...
      tbody.innerHTML = membershipAppsCache.map((a, i) => {
        ...
        return `
          <tr>
            <td>${a.photo_url ? `<img src="${escapeHtml(a.photo_url)}" alt="Photo of ${escapeHtml(a.full_name || 'applicant')}" style="width:34px; height:42px; object-fit:cover; border-radius:3px;">` : '—'}</td>
