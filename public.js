...
import { GITHUB_IMAGES_PATH, GITHUB_OWNER, GITHUB_REPO, compressImageFile, escapeHtml, fetchAndRenderFinances, generateCaptchaCode, parseStoredEventDate, renderCaptchaCanvas, showPage, supabaseClient } from './common.js';
  ...
  // Shared by openLightbox (public gallery) and member.js's openInternalLightbox
  // so both get the same keyboard/focus behavior instead of duplicating it.
  export function displayLightbox(url, altText) {
  ...
  function openLightbox(idx){
    ...
    displayLightbox(IMAGE_DATA[idx].url, IMAGE_DATA[idx].title);
  }
  ...
  function renderSlideshow(images) {
    ...
    slideImages.forEach((img, idx) => {
      slidesHTML += `
        <div class="slide fade-anim" tabindex="0" role="button" aria-label="View larger: ${escapeHtml(img.title || 'photo')}" onclick="openLightbox(${idx})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openLightbox(${idx});}">
  ...
  function renderGalleryBatch() {
    ...
    const html = nextBatch.map((img, localIdx) => {
      ...
      return `
        <div class="art-frame" tabindex="0" role="button" aria-label="View larger: ${escapeHtml(img.title || 'photo')}" onclick="openLightbox(${globalIdx})" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();openLightbox(${globalIdx});}">
      ...
      container.innerHTML = allPosts.map(p => {
        ...
        return `
          <div class="magazine-post">
            ...
            <div class="mp-body" style="font-family:'Work Sans',sans-serif;">${DOMPurify.sanitize(p.content || '')}</div>
          </div>
        `;
      }).join('');
    } catch (err) {
