  ...
  async function generateMemberIdCard(index) {
    ...
    doc.setFontSize(9);
    doc.text("DIHIBALIHARPUR TARUN SANGHA", 47.5, 8, { align: "center" });
    doc.setFontSize(6.5);
    doc.setTextColor(230, 230, 230);
    doc.text("MEMBERSHIP ID CARD", 47.5, 12.5, { align: "center" });

    // Profile photo (or placeholder circle)
    try {
      if (m.photo_url) {
        const photoData = await getBase64ImageFromUrl(m.photo_url);
        if (photoData) doc.addImage(photoData, 'PNG', 6, 17, 24, 24);
      }
    } catch (e) {}
    doc.setDrawColor(224, 162, 51);
    ...
    doc.setFont("Helvetica", "bold");
    doc.setFontSize(10);
    doc.text(String(m.name || ''), 34, 22);
    doc.setFont("Helvetica", "normal");
    doc.setFontSize(7.5);
    doc.setTextColor(224, 162, 51);
    doc.text(String(m.designation || 'Member'), 34, 27);
    doc.setTextColor(210, 210, 210);
    doc.setFontSize(6.5);
    if (m.member_since) doc.text("Member since: " + m.member_since, 34, 32);
    doc.text("Reg. No. S0034236", 34, 37);
    ...
    doc.save(`ID_Card_${String(m.name || 'member').replace(/\s+/g, '_')}.pdf`);
  }
        ...
        return `
          <tr>
            ...
            <td style="white-space:nowrap;">
              ${a.status === 'pending' ? `
                <button onclick="approveApplication('${a.id}')" style="padding:3px 7px; background:var(--leaf); color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:10.5px; margin-right:3px;">Approve</button>
  ...
  function approveApplication(id) { updateApplicationStatus(id, 'approved'); }
